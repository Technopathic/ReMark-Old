...
.controller('MainCtrl', ['$scope', '$state', '$http', '$rootScope', '$mdDialog', '$mdBottomSheet', '$mdToast', '$timeout', 'mainData', function($scope, $state, $http, $rootScope, $mdDialog, $mdBottomSheet, $mdToast, $timeout, mainData) {
  ...
  $scope.closeSubscription = function() {
    $scope.showSubscription = false;
  };

  if($scope.mainOptions.allowSubscription == 1)
  {
    $timeout(function() {
  ...
  $scope.doSubscription = function() {
    $http({
        method: 'POST',
        url: 'api/postSubscription',
        data: {contact:$scope.subscriptionData.email},
        headers: {'Content-Type': 'application/x-www-form-urlencoded'}
    }).success(function(data){
      if(data == 0)
      {
        $scope.notifyToast('Please enter an Email.');
      }
      else if(data == 1)
      {
        $scope.notifyToast('Thanks for following! Be sure to check your email.');
        $scope.closeSubscription();
      }
      else if(data == 2)
      {
        $scope.notifyToast('Admin has not enabled this feature.');
      }
      else if(data == 3)
      {
        $scope.notifyToast('You are already following!');
      }
      else if(data == 4)
      {
        $scope.notifyToast('Following has been disabled.');
      }
    });
  };
...
.controller('DetailCtrl', ['$scope', '$rootScope', '$state', '$stateParams', '$location', '$http', '$mdBottomSheet', '$mdToast', '$mdDialog', 'hotkeys', 'detailData', 'replyData', function($scope, $rootScope, $state, $stateParams, $location, $http, $mdBottomSheet, $mdToast, $mdDialog, hotkeys, detailData, replyData) {
  ...
  $scope.cleanAudio = function() {
    angular.element(document).ready(function() {
      if($scope.topic.topicType == "Audio") {
        audio = videojs("audioPlayer",
        {
          controls: true,
          height:125,
          autoplay: false,
  ...
  $scope.cleanVideo = function() {
    angular.element(document).ready(function() {
      if($scope.topic.topicType == "Video") {
